...
$$\text{Perplexity} = 2^\left(-\frac{1}{N}\sum_{i=1}^{N} \log q(x_i)\right)$$
...
#### Bits-per-Character
It measures the same thing as perpelxity but in terms of character level than token level using bits (base-2 log). This is dependent on language.
...
#### Bits-per-Byte
This is same as perplexity but measured against raw bytes of text instead of characters making it tokenizer agnostic.
